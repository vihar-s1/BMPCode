...
class AntSystem:
    ...
    def simulate(self, maxIterations:int, alpha:float, beta:float, fractionalOrder:float, evaporationRate: float, pheromoneScaleFactor:float, debug:bool=False) -> None:
...
def __main__():
    ...
    AS.simulate(maxIterations=100, alpha=1, beta=1, fractionalOrder=0.1, evaporationRate=0.36, pheromoneScaleFactor=200.0, debug=args.debug)
    end = datetime.now()
